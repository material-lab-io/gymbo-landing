...
const shot = (page, name, scheme, clip) =>
  page.screenshot({ path: `${OUT}/${name}-${scheme}.png`, ...(clip ? { clip } : {}) });
...
for (const scheme of ['light', 'dark']) {
  ...
  await p2.screenshot({ path: `${OUT}/E4-nav-reveal-${scheme}.png` });
  ...
  await p3.screenshot({ path: `${OUT}/E5-pricing-reveal-${scheme}.png` });
  ...
  await p4.screenshot({ path: `${OUT}/E6-footer-form-${scheme}.png` });
