      ...
      >
        ...
        <section className="flex flex-col gap-3">
          ...
          <ul>
            ...
            <li><strong>Product analytics in the app.</strong> We record how the app is used — screens opened, features used, and errors — with PostHog. So we can tell one trainer's sessions apart, this includes <strong>your own</strong> name and phone number as your account identifier, plus, when an action is about a client, the class or payment amount involved — never anything that identifies that client. Your clients' names, phone numbers, contact details, notes, and internal IDs are never sent. You control this under Settings → Privacy. The app contacts PostHog's servers at launch to fetch configuration every time it opens, regardless of this setting; that request discloses your device's IP address and our PostHog project's API key, but no personal or usage data. What the setting does control: when it is off, we send no usage events, make no contact with PostHog's event-collection servers, and none of your name, phone number, or activity leaves the device.</li>
