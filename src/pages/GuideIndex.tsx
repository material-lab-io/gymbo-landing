...
export function GuideIndex() {
  ...
  const card = (p: Post) => (
    <a key={p.slug} href={`/guide/${p.slug}/`} className="group block py-6 transition-opacity hover:opacity-80" style={{ borderTop: "1px solid var(--c-line)" }}>
      <h3 className="text-[20px] md:text-[24px] font-black" style={{ fontFamily: SERIF, letterSpacing: "-0.01em", lineHeight: 1.5, color: F.ink }}>{p.title}</h3>
  ...
  return (
    <PageShell>
      <div className="max-w-[760px] mx-auto px-5 md:px-12 pt-12 md:pt-16 pb-20">
        ...
        <h2 className="mt-16 text-[clamp(22px,3.4vw,32px)] font-black" style={{ fontFamily: SERIF, letterSpacing: "-0.02em", lineHeight: 1.4, color: F.ink }}>
          Quick answers
        </h2>
